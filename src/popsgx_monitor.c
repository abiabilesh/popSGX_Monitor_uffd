...
//Testing macro address for remote execution
#define REMOTE_CALL_INSTR_ADDRESS   0x43c366 
#define POST_REMOTE_CALL_INSTR_ADDRESS  0x43c36b
#define REMOTE_CALL_INSTR_ADDRESS1   0x43c456
#define POST_REMOTE_CALL_INSTR_ADDRESS1  0x43c56b 
#define REMOTE_CALL_INSTR_ADDRESS2   0x43c5e8
#define POST_REMOTE_CALL_INSTR_ADDRESS2  0x43c5ed 
...
int main(int argc, char* argv[]){
    ...
    if(monitor_app.mode == CLIENT || monitor_app.mode == SERVER){
        unsigned long int expected_inst_address = REMOTE_CALL_INSTR_ADDRESS;
        wait(&status);
        ...
        wait(&status);
        log_info("Application hit a breakpoint : %p\n", REMOTE_CALL_INSTR_ADDRESS);
    ...
    if(monitor_app.mode == CLIENT){
        ...
        unsigned long int expected_inst_address = REMOTE_CALL_INSTR_ADDRESS1;
        //wait(&status);
        long old_data = set_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address);
        ptrace(PTRACE_CONT, monitor_app.dsm.child.c_pid, NULL, NULL);
        wait(&status);
        log_info("Client hit a breakpoint %p\n", REMOTE_CALL_INSTR_ADDRESS1);
        clear_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address, old_data);
        ...
        expected_inst_address = REMOTE_CALL_INSTR_ADDRESS2;
        //wait(&status);
        old_data = set_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address);
        ptrace(PTRACE_CONT, monitor_app.dsm.child.c_pid, NULL, NULL);
        wait(&status);
        log_info("Client hit a breakpoint %p\n", REMOTE_CALL_INSTR_ADDRESS2);
        clear_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address, old_data);
        ...
        unsigned long int expected_inst_address = POST_REMOTE_CALL_INSTR_ADDRESS;
        long old_data = set_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address);
        
        ptrace(PTRACE_CONT, monitor_app.dsm.child.c_pid, NULL, NULL);
        wait(&status);
        log_info("Server hit a breakpoint %p\n", POST_REMOTE_CALL_INSTR_ADDRESS);
        clear_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address, old_data);
        ...
        //unsigned long int expected_inst_address = 0x43c411;
        expected_inst_address = POST_REMOTE_CALL_INSTR_ADDRESS1;
        old_data = set_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address);
        
        ptrace(PTRACE_CONT, monitor_app.dsm.child.c_pid, NULL, NULL);
        wait(&status);
        log_info("Server hit a breakpoint %p\n", POST_REMOTE_CALL_INSTR_ADDRESS1);
        clear_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address, old_data);
        ...
        //unsigned long int expected_inst_address = 0x43c411;
        expected_inst_address = POST_REMOTE_CALL_INSTR_ADDRESS2;
        old_data = set_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address);
        
        ptrace(PTRACE_CONT, monitor_app.dsm.child.c_pid, NULL, NULL);
        wait(&status);
        log_info("Server hit a breakpoint %p\n", POST_REMOTE_CALL_INSTR_ADDRESS2);
        clear_breakpoint(monitor_app.dsm.child.c_pid, expected_inst_address, old_data);
        for(int i = 0; i < no_of_shared_pages; i++){
             char page[4096] = "";
            // if(i != no_of_shared_pages - 1){
