...
static uint8_t handle_rw_pagefault(msi_handler* msi, long uffd, struct uffd_msg msg, char *page, int sk);
...
fault_handler_thread(void *arg)
{
	...
	for (;;) {
		...
		uint8_t pagefault_type = handle_rw_pagefault(handler_arg->msi, uffd, msg, page, handler_arg->sk);
		...
		if(pagefault_type == NEW_PAGEFAULT_WRITE || pagefault_type == PAGEFAULT_WRITE_PROTECTION){
			...
			msi_handle_write_command(handler_arg->msi, handler_arg->sk, msg.arg.pagefault.address, t, sysconf(_SC_PAGE_SIZE));
 ...
 */
long setup_userfaultfd_region(msi_handler *msi, pthread_t* thr, void* (*handler)(void*), int sk, int uffd, pid_t victim_pid)
{

    log_debug("setup_userfaultfd_region");
